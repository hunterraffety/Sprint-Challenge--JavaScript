...
const getBothNames = zooAnimals.forEach(element => {
  animalNames.push(
    `Name: ${element.animal_name}, Scientic: ${element.scientific_name}`
  );
});
...
const listNames = zooAnimals.map(
  (name = name => {
...
const getPopulation = zooAnimals.filter(animal => {
  if (animal.population < 5) {
    largerPopulation.push(animal.animal_name);
  }
});
...
*/
const populationTotal = zooAnimals.reduce(function(acc, count) {
